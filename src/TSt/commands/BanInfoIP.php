...
class BanInfoIP extends API{
	public function __construct(Loader $plugin){
        parent::__construct($plugin, "baninfo-ip", "Информация о бане", "/bi-ip <ник>", null, ["bi-ip", "tbi-ip"]);
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if(count($args) === 0){
            $sender->sendMessage("/bi-ip <IP адрес>");

			return false;
		}
    $value = array_shift($args);
    
    $bans = file_get_contents("banned-ips.txt", $sender->getServer()->getDataPath());
